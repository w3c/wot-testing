...
INPUTS="TDs"
OUTPUTS="TD_test_outputs"

# Location of tools directory, which contains a checked out version of
# the AssertionTester from the TD Playground.
TOOLS="../../../tools"

# Suffixes used to identify input files:
SUFFIXES="{td.jsonld}"
...
#      implementation name.
#   $INPUTS/Org/X.$SUFFIXES (+ optional matched csv files) at top level: 
...
#   $INPUTS/Org/S/*.$SUFFIXES (+ optional $INPUTS/Org/S.csv files 
...
for OrgDir in $INPUTS/* ; do
  if [[ -d $OrgDir ]]; then
    ...
    for ImplPath in $AbsOrgDir/*.$SUFFIXES ; do
       if [[ -f $ImplPath ]]; then
    ...
    for ImplPath in $AbsOrgDir/* ; do
       if [[ -d $ImplPath ]]; then
          ...
          for InstancePath in $ImplPath/*.$SUFFIXES ; do
